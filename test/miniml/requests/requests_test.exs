defmodule Miniml.RequestsTest do
  use Miniml.DataCase
  ...
  describe "requests" do
    ...
    @valid_attrs %{full: "some full", minified: "some minified"}
    ...
    test "create_request/1 with valid data creates a request" do
      ...
      assert request.full == "some full"
      assert request.minified == "some minified"
    end
    ...
    test "update_request/2 with valid data updates the request" do
      request = request_fixture()
      assert {:ok, request} = Requests.update_request(request, @update_attrs)
      assert %Request{} = request
      assert request.full == "some updated full"
      assert request.minified == "some updated minified"
    end
